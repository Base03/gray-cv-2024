%-------------------------------------------------------------------------------

\cvsection{Skills}

\begin{cventries}
  \cventryone{Machine Learning}{} %\ratingdefault{4}
  {
    \begin{cvitems}
    %\item PyTorch, Jax, Neural Networks, Deep Learning, Transformer, GRU, LSTM, CNN, RNN, Reinforcement Learning
    \item Neural Networks, Deep Learning, Reinforcement Learning
    \item Attention Mechanisms, Transformers, CNNs, RNNs, GANs, Diffusion Models, GRUs, LSTMs
    \end{cvitems}
  }

  \cventryone{Scientific Computing}{} %\ratingdefault{5}
  {
    \begin{cvitems}
    %\item Numerical Methods, HPC, MPI, OpenMP, CUDA, C, Python, Computational Modeling, Finite Difference, Spectral Methods, Monte Carlo, Optimization, Debugging, Profiling
    \item HPC, Numerical Methods, Computational Modeling
    \item Finite Difference, Finite Element, Finite Volume, Spectral Methods, Monte Carlo, AMR, Multigrid
    \item Optimization, Profiling, Debugging
    \end{cvitems}
  }

  \cventryone{Quantitative Methods}{} %\ratingdefault{5}
  {
    \begin{cvitems}
    %\item Linear Algebra, Calculus, Probability, Field Theory, PDEs, SDEs, ODEs, Optimization
    \item Calculus, PDEs, SDEs, ODEs, Linear Algebra, Probability, Statistics, Signal Processing
    \item Lagrangian Mechanics, Hamiltonian Mechanics
    \item Quantum Mechanics, Statistical Mechanics, Thermodynamics
    \item Optics, Spectroscopy, Computational Optics
    \item Field Theory, Fluid Dynamics, Electromagnetism, General Relativity
    \end{cvitems}
  }

  \cventryone{Languages and Frameworks}{} %\ratingdefault{5}
  {
    \begin{cvitems}
    \item C, Python, Fortran 
    \item Pytorch, Jax, TensorFlow, Pandas
    \item MPI, OpenMP, CUDA
    \end{cvitems}
  }

  \cventryone{Numerical Analysis}{} %\ratingdefault{5}
  {
    \begin{cvitems}
    \item Error analysis, Stability Analysis, Complexity Analysis, Convergence Analysis
    \item Conditioning, Discretization
    \end{cvitems}
  }

  \cventryone{Research}{} %\ratingdefault{5}
  {
    \begin{cvitems}
    %\item Research Design, Data Analysis, Scientific Writing, Literature Review, Data Visualization, Teaching
    \item Research Design, Scientific Writing, Literature Review
    \item Data Analysis, Data Visualization
    \item Mentoring, Curriculum Development, Presentations
    \end{cvitems}
  }

\end{cventries}
